...
**"LLM을 똑똑하게 만든 게 아니라, 말 안 듣게 만들었다."**
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           PM DECISION DFA (v2.5.5)                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   DISPATCH ──→ RETRY ──→ DISPATCH (max 2)                                  │
│       │           │                                                         │
│       ├──→ DONE   └──→ BLOCKED ──→ ESCALATE ──→ DONE                       │
│       │                                                                     │
│       └──→ BLOCKED ──→ ESCALATE ──→ DONE                                   │
│                                                                             │
│   금지된 전이:                                                              │
│   ├─ DISPATCH → ESCALATE ❌ (BLOCKED 경유 필수)                            │
│   ├─ DONE → * ❌ (terminal)                                                 │
│   ├─ RETRY → ESCALATE ❌ (BLOCKED 경유 필수)                               │
│   └─ BLOCKED → DISPATCH ❌ (ESCALATE로만)                                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 원칙 | 설명 |
|------|------|
| **Monotonic** | SELF_REPAIR → ROLE_SWITCH → HARD_FAIL (절대 감소 안함) |
| **Terminal** | HARD_FAIL 도달 후 모든 실패는 HARD_FAIL 유지 |
| **Once** | ROLE_SWITCH는 프로필당 1회만 허용 |
...
| v2.1.1 | 2026-01-05 | 통장 보호 + 주둥이 봉인 |
